...
public class DrawHelper {
	...
	public static void drawSnail(IBrush painter, double h, double w, double sizeInPercent) {
		painter.save();
		painter.turn(75);
		double size = sizeInPercent / 2 + 0.5;
		double length = size * h * .4;
		double width = size * h * .4;
		painter.jump(-length / 3);
		painter.shape().ellipse().width(length).height(width).fillPaint(Color.GREEN).fill();
		// painter.jump(length / 2);
